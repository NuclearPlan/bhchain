...
func GetTxCmd(storeKey string, cdc *codec.Codec) *cobra.Command {
	...
	openswapTxCmd.AddCommand(client.PostCommands(
		...
		GetCmdClaimLPEarning(cdc),
		GetCmdClaimTradeEarning(cdc),
	)...)
...
func GetCmdAddLiquidity(cdc *codec.Codec) *cobra.Command {
	...
	cmd.Flags().String(FlagMinTokenAAmount, "", "The min amount of the first token")
	cmd.Flags().String(FlagMinTokenBAmount, "", "The min amount of the second token")
	...
	cmd.MarkFlagRequired(FlagMinTokenAAmount)
	cmd.MarkFlagRequired(FlagMinTokenBAmount)
...
func GetCmdClaimLPEarning(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "claimlp",
		Short: "claim lp earning of a trading pair",
		RunE: func(cmd *cobra.Command, args []string) error {
			txBldr := custodianunit.NewTxBuilderFromCLI().WithTxEncoder(utils.GetTxEncoder(cdc))
			cliCtx := context.NewCLIContext().WithCodec(cdc)

			msg, err := buildClaimLPEarningMsg(cliCtx)
			if err != nil {
				return err
			}
...
func GetCmdClaimTradeEarning(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "claimtrade",
		Short: "claim trade earning of a trading pair",
		RunE: func(cmd *cobra.Command, args []string) error {
			txBldr := custodianunit.NewTxBuilderFromCLI().WithTxEncoder(utils.GetTxEncoder(cdc))
			cliCtx := context.NewCLIContext().WithCodec(cdc)

			msg, err := buildClaimTradeEarningMsg(cliCtx)
			if err != nil {
				return err
			}
