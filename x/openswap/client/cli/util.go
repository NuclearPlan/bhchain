...
func buildAddLiquidityMsg(cliCtx context.CLIContext) (sdk.Msg, error) {
	...
	minTokenAAmt, ok := sdk.NewIntFromString(viper.GetString(FlagMinTokenAAmount))
	if !ok {
		return nil, errors.New("invalid min token a amount")
	}
	minTokenBAmt, ok := sdk.NewIntFromString(viper.GetString(FlagMinTokenBAmount))
	if !ok {
		return nil, errors.New("invalid token b amount")
	}
	expiredAt := viper.GetInt64(FlagExpiredTime)
	msg := types.NewMsgAddLiquidity(from, viper.GetUint32(FlagDexID), tokenA, tokenB, tokenAAmt, tokenBAmt, minTokenAAmt, minTokenBAmt, expiredAt)
	if err := msg.ValidateBasic(); err != nil {
		return nil, err
	}
	return msg, nil
...
func buildRemoveLiquidityMsg(cliCtx context.CLIContext) (sdk.Msg, error) {
	...
	expiredAt := viper.GetInt64(FlagExpiredTime)
	msg := types.NewMsgRemoveLiquidity(from, viper.GetUint32(FlagDexID), tokenA, tokenB, liquidity, minTokenAAmt, minTokenBAmt, expiredAt)
	if err := msg.ValidateBasic(); err != nil {
		return nil, err
	}
	return msg, nil
...
func buildClaimLPEarningMsg(cliCtx context.CLIContext) (sdk.Msg, error) {
	from := cliCtx.GetFromAddress()
	tokenA := sdk.Symbol(viper.GetString(FlagTokenA))
	tokenB := sdk.Symbol(viper.GetString(FlagTokenB))
	msg := types.NewMsgClaimLPEarning(from, viper.GetUint32(FlagDexID), tokenA, tokenB)
	if err := msg.ValidateBasic(); err != nil {
		return nil, err
	}
	return msg, nil
...
func buildClaimTradeEarningMsg(cliCtx context.CLIContext) (sdk.Msg, error) {
	from := cliCtx.GetFromAddress()
	tokenA := sdk.Symbol(viper.GetString(FlagTokenA))
	tokenB := sdk.Symbol(viper.GetString(FlagTokenB))
	msg := types.NewMsgClaimTradeEarning(from, viper.GetUint32(FlagDexID), tokenA, tokenB)
	if err := msg.ValidateBasic(); err != nil {
		return nil, err
	}
	return msg, nil
