...
func NewResTradingPair(pair *TradingPair, reward *TradeReward) *ResTradingPair {
	res := &ResTradingPair{
		...
		UnclaimedTradingReward: sdk.ZeroInt(),
		UnclaimedTradingVolume: sdk.ZeroInt(),
	}
	if reward != nil {
		res.UnclaimedTradingReward = reward.Reward
		res.UnclaimedTradingVolume = reward.TradingVolume
	}
	return res
}
...
func NewResTradingPairs(pairs []*TradingPair) []*ResTradingPair {
	...
	for i := range pairs {
		ret[i] = NewResTradingPair(pairs[i], nil)
	}
	return ret
}
