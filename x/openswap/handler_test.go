...
import (
	...
	"github.com/bluehelix-chain/bhchain/x/mint"
	...
	"github.com/bluehelix-chain/bhchain/x/params/subspace"
	"github.com/bluehelix-chain/bhchain/x/receipt"
	"github.com/bluehelix-chain/bhchain/x/supply"
	"github.com/bluehelix-chain/bhchain/x/token"
...
func TestHandleMsgAddLiquidity(t *testing.T) {
	...
	msg := types.NewMsgAddLiquidity(address, 0, "fakebtc", "usdt", sdk.NewInt(20000), sdk.NewInt(8000000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	...
	msg = types.NewMsgAddLiquidity(address, 0, "usdt", "fakebtc", sdk.NewInt(20000), sdk.NewInt(8000000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx.WithMultiStore(ctx.MultiStore()), k, msg)
	assert.Equal(t, sdk.CodeUnsupportToken, res.Code)
	assert.Contains(t, res.Log, "token fakebtc does not exist")
	...
	msg = types.NewMsgAddLiquidity(address, 0, "usdt", "btc", sdk.NewInt(20000), sdk.NewInt(8000000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx.WithMultiStore(ctx.MultiStore()), k, msg)
	assert.Equal(t, sdk.CodeUnsupportToken, res.Code)
	assert.Contains(t, res.Log, "token btc is not enable to send")

	msg = types.NewMsgAddLiquidity(address, 0, "btc", "usdt", sdk.NewInt(20000), sdk.NewInt(8000000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx.WithMultiStore(ctx.MultiStore()), k, msg)
	assert.Equal(t, sdk.CodeUnsupportToken, res.Code)
	assert.Contains(t, res.Log, "token btc is not enable to send")
	...
	// test expired tx
	msg = types.NewMsgAddLiquidity(address, 0, "btc", "usdt", sdk.NewInt(20000), sdk.NewInt(8000000), sdk.ZeroInt(), sdk.ZeroInt(), 1)
	res = handleMsgAddLiquidity(ctx.WithMultiStore(ctx.MultiStore()), k, msg)
	assert.Equal(t, sdk.CodeInvalidTx, res.Code)
	assert.Contains(t, res.Log, "expired tx")

	// test liquidity too low
	msg = types.NewMsgAddLiquidity(address, 0, "btc", "usdt", sdk.NewInt(200), sdk.NewInt(800), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx.WithMultiStore(ctx.MultiStore()), k, msg)
	assert.Equal(t, sdk.CodeAmountError, res.Code)
	assert.Contains(t, res.Log, "insufficient liquidity")

	// test insufficient funds
	msg = types.NewMsgAddLiquidity(address, 0, "btc", "usdt", sdk.NewInt(20000), sdk.NewInt(800000000000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	...
	// test slippage too big
	msg = types.NewMsgAddLiquidity(address, 0, "usdt", "btc", sdk.NewInt(20000), sdk.NewInt(800), sdk.NewInt(20000), sdk.NewInt(51), 999999999999)
	res = handleMsgAddLiquidity(ctx, k, msg)
	assert.Equal(t, sdk.CodeInvalidTx, res.Code)
	assert.Contains(t, res.Log, "amount not enough")

	// test success
	msg = types.NewMsgAddLiquidity(address, 0, "usdt", "btc", sdk.NewInt(20000), sdk.NewInt(800), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
...
func TestHandleMsgRemoveLiquidity(t *testing.T) {
	...
	msg := types.NewMsgRemoveLiquidity(address, 0, "btc", "usdt", sdk.NewInt(30000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res := handleMsgRemoveLiquidity(ctx, k, msg)
	assert.Equal(t, sdk.CodeInvalidTx, res.Code)
	assert.Contains(t, res.Log, "btc-usdt trading pair does not exist")

	addMsg := types.NewMsgAddLiquidity(address, 0, "btc", "usdt", sdk.NewInt(20000), sdk.NewInt(8000000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx, k, addMsg)
	assert.True(t, res.IsOK())

	// test token not exists
	msg = types.NewMsgRemoveLiquidity(address, 0, "fakebtc", "usdt", sdk.NewInt(30000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgRemoveLiquidity(ctx, k, msg)
	assert.Equal(t, sdk.CodeUnsupportToken, res.Code)
	assert.Contains(t, res.Log, "token fakebtc does not exist")

	msg = types.NewMsgRemoveLiquidity(address, 0, "usdt", "fakebtc", sdk.NewInt(30000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgRemoveLiquidity(ctx, k, msg)
	assert.Equal(t, sdk.CodeUnsupportToken, res.Code)
	assert.Contains(t, res.Log, "token fakebtc does not exist")
	...
	btc.SendEnabled = false
	input.tk.SetToken(ctx, btc)
	msg = types.NewMsgRemoveLiquidity(address, 0, "usdt", "btc", sdk.NewInt(30000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgRemoveLiquidity(ctx, k, msg)
	assert.Equal(t, sdk.CodeUnsupportToken, res.Code)
	assert.Contains(t, res.Log, "token btc is not enable to send")
	...
	ctx = ctx.WithBlockTime(time.Unix(1000, 0))
	// test expired tx
	msg = types.NewMsgRemoveLiquidity(address, 0, "btc", "usdt", sdk.NewInt(30000), sdk.ZeroInt(), sdk.ZeroInt(), 1)
	res = handleMsgRemoveLiquidity(ctx, k, msg)
	assert.Equal(t, sdk.CodeInvalidTx, res.Code)
	assert.Contains(t, res.Log, "expired tx")

	// test liquidity too big
	msg = types.NewMsgRemoveLiquidity(address, 0, "btc", "usdt", sdk.NewInt(300000000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgRemoveLiquidity(ctx, k, msg)
	...
	// test slippage too big
	msg = types.NewMsgRemoveLiquidity(address, 0, "btc", "usdt", sdk.NewInt(30000), sdk.NewInt(20000), sdk.ZeroInt(), 999999999999)
	res = handleMsgRemoveLiquidity(ctx, k, msg)
	assert.Equal(t, sdk.CodeInvalidTx, res.Code)
	assert.Contains(t, res.Log, "amount not enough")
	...
	// 赎回全部流动性
	msg = types.NewMsgRemoveLiquidity(address, 0, "usdt", "btc", sdk.NewInt(369000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
...
func TestHandleMsgSwapFail(t *testing.T) {
	...
	res := handleMsgAddLiquidity(ctx, k, addMsg)
	assert.True(t, res.IsOK())
	addMsg = types.NewMsgAddLiquidity(address, 0, "eth", "usdt", sdk.NewInt(40000), sdk.NewInt(8000000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx, k, addMsg)
	assert.True(t, res.IsOK())
...
func TestHandleMsgSwapExactInSuccess(t *testing.T) {
	...
	addMsg := types.NewMsgAddLiquidity(address, 0, "btc", "usdt", btcUsdtAmountBtc, btcUsdtAmountUsdt, sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res := handleMsgAddLiquidity(ctx, k, addMsg)
	assert.True(t, res.IsOK())

	ethUsdtAmountEth := sdk.NewInt(40000)
	ethUsdtAmountUsdt := sdk.NewInt(8000000)
	addMsg = types.NewMsgAddLiquidity(address, 0, "eth", "usdt", ethUsdtAmountEth, ethUsdtAmountUsdt, sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx, k, addMsg)
	assert.True(t, res.IsOK())
...
func TestHandleMsgSwapEdgeCase(t *testing.T) {
	...
	addMsg = types.NewMsgAddLiquidity(address, 0, "btc", "eth", btcEthAmountBtc, btcEthAmountEth, sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx, k, addMsg)
	assert.True(t, res.IsOK())
...
func TestHandleMsgCreateTradingPair(t *testing.T) {
	...
	addMsg := types.NewMsgAddLiquidity(lpAddr, 0, "btc", "usdt", sdk.NewInt(1000), sdk.NewInt(10000000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx, k, addMsg)
	assert.True(t, res.IsOK())
	ctx = ctx.WithEventManager(sdk.NewEventManager())
	...
	addMsg = types.NewMsgAddLiquidity(lpAddr, 0, "eth", "usdt", sdk.NewInt(1000), sdk.NewInt(10000000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx, k, addMsg)
	assert.True(t, res.IsOK())
	ctx = ctx.WithEventManager(sdk.NewEventManager())
...
func TestHandleMsgEditTradingPair(t *testing.T) {
	...
	addMsg := types.NewMsgAddLiquidity(lpAddr, 1, "btc", "eth", sdk.NewInt(1000), sdk.NewInt(10000000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx, k, addMsg)
	assert.True(t, res.IsOK())
...
func TestHandleMsgAddLiquidityInCustomDex(t *testing.T) {
	...
	// test trading pair not exists
	msg := types.NewMsgAddLiquidity(address, 1, "btc", "usdt", sdk.NewInt(20000), sdk.NewInt(8000000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx, k, msg)
	assert.Equal(t, sdk.CodeInvalidTx, res.Code)
	assert.Contains(t, res.Log, "trading pair does not exist in dex 1")
	...
	msg = types.NewMsgAddLiquidity(address, 1, "eth", "usdt", sdk.NewInt(20000), sdk.NewInt(8000000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx, k, msg)
	assert.True(t, res.IsOK())
...
func TestHandleMsgRemoveLiquidityInCustomDex(t *testing.T) {
	...
	addMsg := types.NewMsgAddLiquidity(address, 0, "btc", "usdt", sdk.NewInt(10000), sdk.NewInt(4000000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res := handleMsgAddLiquidity(ctx, k, addMsg)
	assert.True(t, res.IsOK())
	...
	// test trading pair not exists
	msg := types.NewMsgRemoveLiquidity(address, 1, "btc", "usdt", sdk.NewInt(30000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgRemoveLiquidity(ctx, k, msg)
	assert.Equal(t, sdk.CodeInvalidTx, res.Code)
	assert.Contains(t, res.Log, "trading pair does not exist in dex 1")
	...
	addMsg = types.NewMsgAddLiquidity(address, 1, "btc", "usdt", sdk.NewInt(20000), sdk.NewInt(8000000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx, k, addMsg)
	assert.True(t, res.IsOK())
...
func TestSwapRefererReward(t *testing.T) {
	...
	addMsg = types.NewMsgAddLiquidity(address, 1, "btc", "usdt", sdk.NewInt(20000), sdk.NewInt(4000000), sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx, k, addMsg)
	assert.True(t, res.IsOK())
...
func TestSwapLpReward(t *testing.T) {
	...
	addMsg := types.NewMsgAddLiquidity(lpAddr, 0, "btc", "usdt", addAmountBtc, addAmountUsdt, sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res := handleMsgAddLiquidity(ctx, k, addMsg)
	assert.True(t, res.IsOK())
	...
	addMsg = types.NewMsgAddLiquidity(lpAddr2, 1, "btc", "usdt", addAmountBtc2, addAmountUsdt2, sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx, k, addMsg)
	assert.True(t, res.IsOK())
	...
	removeMsg := types.NewMsgRemoveLiquidity(lpAddr, 1, "btc", "usdt", liquidityLp1, sdk.ZeroInt(), sdk.ZeroInt(), 0)
	...
	removeMsg = types.NewMsgRemoveLiquidity(lpAddr2, 0, "btc", "usdt", liquidityLp2, sdk.ZeroInt(), sdk.ZeroInt(), 0)
	res = handleMsgRemoveLiquidity(ctx, k, removeMsg)
	assert.True(t, res.IsOK())
	...
	addAmountUsdt = sdk.NewInt(4000000)
	addMsg = types.NewMsgAddLiquidity(lpAddr3, 1, "eth", "usdt", addAmountEth, addAmountUsdt, sdk.ZeroInt(), sdk.ZeroInt(), 999999999999)
	res = handleMsgAddLiquidity(ctx, k, addMsg)
	assert.True(t, res.IsOK())
	...
	removeMsg = types.NewMsgRemoveLiquidity(lpAddr3, 1, "eth", "usdt", liquidityLp3, sdk.ZeroInt(), sdk.ZeroInt(), 0)
	res = handleMsgRemoveLiquidity(ctx, k, removeMsg)
	assert.True(t, res.IsOK())
