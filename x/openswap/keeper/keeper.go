...
import (
	...
	"github.com/bluehelix-chain/bhchain/x/openswap/orderbook"
...
func (k Keeper) AddLiquidity(ctx sdk.Context, from sdk.CUAddress, dexID uint32, tokenA, tokenB sdk.Symbol,
	...
	if pair == nil || pair.TotalLiquidity.IsZero() {
		product := big.NewInt(0).Mul(tokenAAmount.BigInt(), tokenBAmount.BigInt())
		...
		needTokenA, needTokenB = tokenAAmount, tokenBAmount
	} else {
		needTokenB = mulAndDiv(tokenAAmount, pair.TokenBAmount, pair.TokenAAmount)
		if needTokenB.GT(tokenBAmount) {
			needTokenA = mulAndDiv(tokenBAmount, pair.TokenAAmount, pair.TokenBAmount)
			if needTokenA.GT(tokenAAmount) {
				return sdk.ErrInvalidAmount("need amount exceeds expectation").Result()
			}
			needTokenB = tokenBAmount
		} else {
			needTokenA = tokenAAmount
		}
	...
	if needTokenA.LT(minTokenAAmount) || needTokenB.LT(minTokenBAmount) {
		return sdk.ErrInvalidTx(fmt.Sprintf("amount not enough, got: %s %s, want: %s %s",
			needTokenA.String(), needTokenB.String(), minTokenAAmount.String(), minTokenBAmount.String())).Result()
	}
	if !liquidity.IsPositive() {
...
func (k Keeper) RemoveLiquidity(ctx sdk.Context, from sdk.CUAddress, dexID uint32, tokenA, tokenB sdk.Symbol, liquidity sdk.Int,
	...
	if returnTokenA.LT(minTokenAAmount) || returnTokenB.LT(minTokenBAmount) {
		return sdk.ErrInvalidTx(fmt.Sprintf("return amount not enough, got: %s %s, want: %s %s",
			returnTokenA.String(), returnTokenB.String(), minTokenAAmount.String(), minTokenBAmount.String())).Result()
	}
...
func (k Keeper) directSwap(ctx sdk.Context, dexID uint32, from, referer, receiver sdk.CUAddress, amountIn sdk.Int, path []sdk.Symbol) sdk.Result {
	...
	for i := 0; i < len(path)-1; i++ {
		...
		amountOut, bonus, repurchaseFund, updatedPair := k.swap(ctx, feeRate, pair, path[i], amountIn, false, from)
...
func (k Keeper) swap(ctx sdk.Context, feeRate *types.FeeRate, pair *types.TradingPair, tokenIn sdk.Symbol, amountIn sdk.Int,
	isRepurchasing bool, from sdk.CUAddress) (sdk.Int, sdk.Int, sdk.Int, *types.TradingPair) {
	...
	k.SaveTradingPair(ctx, pair)
	if !isRepurchasing {
		k.onTrade(ctx, pair.DexID, pair.TokenA, pair.TokenB, from, tradeVolume)
	}
...
func (k Keeper) limitSwap(ctx sdk.Context, order *types.Order, pair *types.TradingPair) ([]sdk.Flow, *types.EventSwap, *types.TradingPair, bool) {
	...
	amountOut, bonus, repurchaseFund, pair := k.swap(ctx, order.FeeRate, pair, tokenIn, realMaxAmountIn, false, order.From)
...
func (k Keeper) Repurchase(ctx sdk.Context) sdk.Int {
	...
	repurchaseRoutingToken := k.RepurchaseRoutingToken(ctx)
	for _, coin := range repurchaseFunds {
		switch coin.Denom {
		case repurchaseToken:
			totalRepurchaseAmount = totalRepurchaseAmount.Add(coin.Amount)
		case repurchaseRoutingToken:
			pair := k.GetTradingPair(ctx, 0, sdk.Symbol(coin.Denom), sdk.Symbol(repurchaseToken))
			if pair != nil {
				feeRate := k.getFeeRates(ctx, pair)
				amount, _, _, _ := k.swap(ctx, feeRate, pair, sdk.Symbol(coin.Denom), coin.Amount, true, nil)
				totalRepurchaseAmount = totalRepurchaseAmount.Add(amount)
			}
		default:
			if repurchaseRoutingToken != "" {
				pair := k.GetTradingPair(ctx, 0, sdk.Symbol(coin.Denom), sdk.Symbol(repurchaseRoutingToken))
				if pair != nil {
					feeRate := k.getFeeRates(ctx, pair)
					amount, _, _, _ := k.swap(ctx, feeRate, pair, sdk.Symbol(coin.Denom), coin.Amount, true, nil)
					pair = k.GetTradingPair(ctx, 0, sdk.Symbol(repurchaseToken), sdk.Symbol(repurchaseRoutingToken))
					if pair != nil {
						feeRate = k.getFeeRates(ctx, pair)
						amount, _, _, _ = k.swap(ctx, feeRate, pair, sdk.Symbol(repurchaseRoutingToken), amount, true, nil)
						totalRepurchaseAmount = totalRepurchaseAmount.Add(amount)
					}
				}
			}
