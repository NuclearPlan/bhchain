...
func (k Keeper) canRepurchase(ctx sdk.Context, tokenIn sdk.Symbol, amount sdk.Int) bool {
	...
	if k.GetTradingPair(ctx, 0, tokenIn, repurchaseToken) != nil {
		return true
	}
	repurchaseRoutingToken := sdk.Symbol(k.RepurchaseRoutingToken(ctx))
	if k.GetTradingPair(ctx, 0, tokenIn, repurchaseRoutingToken) == nil {
		return false
	}
	return k.GetTradingPair(ctx, 0, repurchaseToken, repurchaseRoutingToken) != nil
}
