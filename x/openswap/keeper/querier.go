...
func queryTradingPair(ctx sdk.Context, req abci.RequestQuery, k Keeper) ([]byte, sdk.Error) {
	...
	reward := k.GetPairTradeReward(ctx, params.DexID, pair.TokenA, pair.TokenB)
	bz, err := codec.MarshalJSONIndent(k.cdc, types.NewResTradingPair(pair, reward))
	if err != nil {
		return nil, sdk.ErrInternal(sdk.AppendMsgToErr("could not marshal result to JSON", err.Error()))
	}
	return bz, nil
...
func queryUnclaimedEarnings(ctx sdk.Context, req abci.RequestQuery, k Keeper) ([]byte, sdk.Error) {
	...
	lpEarnings := make([]*types.Earning, 0, len(liquidities))
	for _, liquidity := range liquidities {
		amount := k.CalculateLPEarning(ctx, params.Addr, liquidity.DexID, liquidity.TokenA, liquidity.TokenB)
		lpEarnings = append(lpEarnings, types.NewEarning(liquidity.DexID, liquidity.TokenA, liquidity.TokenB, amount, sdk.ZeroInt()))
	}

	tradeEarnings := make([]*types.Earning, 0, len(liquidities))
	store := ctx.KVStore(k.storeKey)
	iter := sdk.KVStorePrefixIterator(store, types.AddrTradeVolumeKeyWithAddrPrefix(params.Addr))
	for ; iter.Valid(); iter.Next() {
		dexID, tokenA, tokenB := types.DecodeAddrTradeVolumeKey(iter.Key())
		amount, addrVolume, _ := k.CalculateTradeEarning(ctx, params.Addr, dexID, tokenA, tokenB)
		tradeEarnings = append(tradeEarnings, types.NewEarning(dexID, tokenA, tokenB, amount, addrVolume))
	}
	iter.Close()

	ret := map[string][]*types.Earning{
		"lp_reward":    lpEarnings,
		"trade_reward": tradeEarnings,
	}
