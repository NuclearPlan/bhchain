...
func handleMsgSubmitProposal(ctx sdk.Context, keeper Keeper, msg MsgSubmitProposal) sdk.Result {
	if keeper.ProposalOnlyKeyNode(ctx) {
		b, _ := keeper.sk.IsActiveKeyNode(ctx, msg.Proposer)
		if !b {
			return sdk.ErrUnauthorized("Only keynode can propose").Result()
		}
	}
	...
	minVoteTime := keeper.GetVotingParams(ctx).MinVoteTime
	if msg.VoteTime < uint32(minVoteTime) {
		errMsg := fmt.Sprintf("Votetime %d Less than MinVoteTime %d", msg.VoteTime, minVoteTime)
		return sdk.ErrInvalidTx(errMsg).Result()
	}
