...
import (
	...
	gcutils "github.com/bluehelix-chain/bhchain/x/gov/client/utils"
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			on, _, err := cliCtx.QueryWithData(fmt.Sprintf("custom/%s/params/onlykeynode", queryRoute), nil)
			if err != nil {
				return err
			}
			...
			var b bool
			cdc.MustUnmarshalJSON(on, &b)

			return cliCtx.PrintOutput(types.NewParams(votingParams, tallyParams, depositParams, b))
		},
	}
}
