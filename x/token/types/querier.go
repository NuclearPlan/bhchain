...
func NewResToken(token sdk.Token, collectFeeAmt, withDrawalFeeAmt, chainGasPrice sdk.Int) *ResToken {
	...
	if token.IsIBCToken() {
		...
		ret.GasPrice = chainGasPrice
		ret.Confirmations = ibcToken.Confirmations
		ret.IsNonceBased = ibcToken.IsNonceBased
		ret.NeedCollectFee = ibcToken.NeedCollectFee
		ret.SysTransferAmount = ibcToken.SysTransferAmount(chainGasPrice)
		ret.OpCUSysTransferAmount = ibcToken.OpCUSysTransferAmount(chainGasPrice)
		ret.WithdrawalFee = withDrawalFeeAmt
		ret.CollectFee = collectFeeAmt
		ret.MappingSymbol = ibcToken.MappingSymbol
	}
	return ret
}
