...
func (keeper BaseKeeper) SysTransfer(ctx sdk.Context, fromCUAddr, toCUAddr sdk.CUAddress, toAddr, orderID, symbol string) sdk.Result {
	...
	if toCUAst.GetCUType() == sdk.CUTypeUser {
		...
		if waitCollectNum.GT(sdk.ZeroInt()) {
			...
			waitCollectNum = sdk.CalcAmountWithDecimalDiff(waitCollectNum, mapTokenInfo.GetDecimals(), tokenInfo.GetDecimals())
			_, flow, err := keeper.AddCoin(ctx, toCUAddr, sdk.NewCoin(tokenInfo.MappingSymbol.String(), waitCollectNum))
			if err != nil {
				return err.Result()
			}
			balanceFlows = append(balanceFlows, flow)

			collectFee := keeper.tk.GetCollectFee(ctx, sdk.Symbol(symbol))
			_, flow, err = keeper.SubCoin(ctx, toCUAddr, collectFee)
			if err != nil {
				return err.Result()
			}
			balanceFlows = append(balanceFlows, flow)
			...
			collectOrder.CostFee = keeper.tk.RawCollectFee(ctx, tokenInfo, chainTokenInfo)
	...
	if toCUAst.GetCUType() == sdk.CUTypeOp {
		if toCUAst.GetMigrationStatus() == sdk.MigrationFinish {
			amount = tokenInfo.OpCUSysTransferAmount(chainTokenInfo.GasPrice)
		} else {
			amount = tokenInfo.GasPrice.Mul(tokenInfo.GasLimit)
		}
	} else {
		amount = tokenInfo.SysTransferAmount(chainTokenInfo.GasPrice)
	}
