...
func (keeper BaseKeeper) CollectFinish(ctx sdk.Context, fromCUAddr sdk.CUAddress, orderIDs []string, costFee sdk.Int, success bool) sdk.Result {
	...
	confirmedFirstTime, _, _ := keeper.evidenceKeeper.Vote(ctx, collectOrders[0].ExtTxHash, fromCUAddr, types.NewTxVote(costFee.Int64(), success), uint64(ctx.BlockHeight()))
	result := sdk.Result{}
	...
	case sdk.AccountBased:
		...
		if !success {
			userCUAst.AddAssetCoins(totalCoins)
		}
		...
		for i := range collectOrders {
			if success {
				...
				_ = keeper.ik.SetDepositStatus(ctx, collectOrders[i].Symbol, collectOrders[i].CollectFromCU, collectOrders[i].Txhash, collectOrders[i].Index, sdk.DepositItemStatusWaitCollect)
				collectOrders[i].Status = sdk.OrderStatusFailed
			}
			keeper.ok.SetOrder(ctx, collectOrders[i])
		}
	...
	flows = append(flows, keeper.rk.NewCollectFinishFlow(orderIDs, costFee, success))
	receipt := keeper.rk.NewReceipt(sdk.CategoryTypeCollect, flows)
	keeper.rk.SaveReceiptToResult(receipt, &result)
...
func (keeper BaseKeeper) Recollect(ctx sdk.Context, fromCUAddr sdk.CUAddress, orderIDs []string) sdk.Result {
	bValidator, _ := keeper.sk.IsActiveKeyNode(ctx, fromCUAddr)
	if !bValidator {
		return sdk.ErrInvalidTx(fmt.Sprintf("recollect from not a validator :%v", fromCUAddr)).Result()
	}
	...
	for _, orderID := range orderIDs {
		...
		if !valid {
			return sdk.ErrInvalidOrder(fmt.Sprintf("order %s is not a Collect Order", orderID)).Result()
		}

		if collectOrder.Status != sdk.OrderStatusFailed {
			return sdk.ErrInvalidOrder(fmt.Sprintf("collect order %s does not fail", orderID)).Result()
		}

		order.SetOrderStatus(sdk.OrderStatusBegin)
		keeper.ok.SetOrder(ctx, order)
		keeper.ok.AddProcessOrder(ctx, order)
	}

	result := sdk.Result{}
	var flows []sdk.Flow
	flows = append(flows, keeper.rk.NewOrderFlow("", sdk.CUAddress{}, "", sdk.OrderTypeCollect, sdk.OrderStatusBegin))
	flows = append(flows, keeper.rk.NewRecollectFlow(orderIDs))
	receipt := keeper.rk.NewReceipt(sdk.CategoryTypeCollect, flows)
	keeper.rk.SaveReceiptToResult(receipt, &result)
	return result
}
