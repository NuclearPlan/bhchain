...
func (keeper BaseKeeper) Withdrawal(ctx sdk.Context, fromCUAddr sdk.CUAddress, toAddr, orderID, symbol string, amt, gasFee sdk.Int) sdk.Result {
	...
	withDrawalFee := keeper.tk.GetWithDrawalFee(ctx, sdk.Symbol(symbol))
	if gasFee.LT(withDrawalFee.Amount) {
		return sdk.ErrInsufficientFee(fmt.Sprintf("need:%v, actual have:%v", withDrawalFee.Amount, gasFee)).Result()
	}

	opcus := keeper.ck.GetOpCUs(ctx, symbol)
	mapTokenInfo := keeper.tk.GetToken(ctx, tokenInfo.MappingSymbol)
	realAmt := sdk.CalcAmountWithDecimalDiff(amt, tokenInfo.GetDecimals(), mapTokenInfo.GetDecimals())
	found := false
	for _, opcu := range opcus {
		opcuAst := keeper.ik.GetCUIBCAsset(ctx, opcu.GetAddress())
		if opcuAst.GetAssetCoins().AmountOf(symbol).GTE(realAmt) {
			found = true
			break
		}
	}
	...
	if !found {
		return sdk.ErrInvalidAmount(fmt.Sprintf("not enought coin [%v, %v]for withdrawal", symbol, amt)).Result()
	}

	feeCoins := sdk.NewCoins(sdk.NewCoin(withDrawalFee.Denom, gasFee))
	coins := sdk.NewCoins(sdk.NewCoin(tokenInfo.MappingSymbol.String(), amt))
	...
	withdrawalOrder := keeper.ok.NewOrderWithdrawal(ctx, fromCUAddr, orderID, symbol, realAmt, gasFee, sdk.ZeroInt(), canonicalToAddr, "", "")
	...
	flows = append(flows, keeper.rk.NewWithdrawalFlow(orderID, fromCUAddr.String(), canonicalToAddr, symbol, realAmt, gasFee, withdrawalOrder.WithdrawStatus))
...
func (keeper BaseKeeper) WithdrawalConfirm(ctx sdk.Context, fromCUAddr sdk.CUAddress, orderID string, valid bool) sdk.Result {
	...
	if confirmedFirstTime {
		...
		if valid {
			...
			mapTokenInfo := keeper.tk.GetToken(ctx, tokenInfo.MappingSymbol)
			amount = sdk.CalcAmountWithDecimalDiff(amount, mapTokenInfo.GetDecimals(), tokenInfo.GetDecimals())

			chainTokenInfo := keeper.tk.GetIBCToken(ctx, tokenInfo.Chain)
			feeCoins := sdk.NewCoins(sdk.NewCoin(chainTokenInfo.MappingSymbol.String(), withdrawOrder.GasFee))
			withdrawCoins := sdk.NewCoins(sdk.NewCoin(tokenInfo.MappingSymbol.String(), amount))
...
func (keeper BaseKeeper) WithdrawalWaitSign(ctx sdk.Context, opCUAddr sdk.CUAddress, orderIDs []string, signHashes [][]byte, rawData []byte) sdk.Result {
	...
	case sdk.AccountBased:
		...
		chainMapTokenInfo := keeper.tk.GetToken(ctx, chainTokenInfo.MappingSymbol)
		realGasFee := sdk.CalcAmountWithDecimalDiff(withdrawalOrder.GasFee, chainTokenInfo.GetDecimals(), chainMapTokenInfo.GetDecimals())
		if costFee.GT(realGasFee) {
...
func (keeper BaseKeeper) WithdrawalFinish(ctx sdk.Context, fromCUAddr sdk.CUAddress, orderIDs []string, costFee sdk.Int, valid bool) sdk.Result {
	...
	confirmedFirstTime, _, _ := keeper.evidenceKeeper.Vote(ctx, withdrawalOrder.Txhash, fromCUAddr, types.NewTxVote(costFee.Int64(), valid), uint64(ctx.BlockHeight()))
	result := sdk.Result{}
	if !confirmedFirstTime {
		return result
	}
	...
	case sdk.AccountBased:
		...
		if valid {
			...
			opCUAst.AddAssetCoins(coins.Sub(sdk.NewCoins(sdk.NewCoin(chain, costFee))))
		}
		...
		if chain == symbol {
			...
			chainTokenInfo := keeper.tk.GetIBCToken(ctx, sdk.Symbol(chain))
			chainMapTokenInfo := keeper.tk.GetToken(ctx, chainTokenInfo.MappingSymbol)
			realGasFee := sdk.CalcAmountWithDecimalDiff(order.GasFee, chainTokenInfo.GetDecimals(), chainMapTokenInfo.GetDecimals())
			opCUAst.AddGasReceived(sdk.NewCoins(sdk.NewCoin(chain, realGasFee)))
		}
		opCUAst.AddGasUsed(sdk.NewCoins(sdk.NewCoin(chain, costFee)))
		if tokenInfo.IsNonceBased {
	...
	for _, orderID := range orderIDs {
		...
		/*	mapChainTokenInfo := keeper.tk.GetToken(ctx, chainTokenInfo.MappingSymbol)
			costFee := sdk.CalcAmountWithDecimalDiff(costFee, mapChainTokenInfo.GetDecimals(), chainTokenInfo.GetDecimals())
			//back gasfee if it is the mainnet asset
			if withdrawalOrder.GasFee.GT(costFee) && withdrawalOrder.Symbol == chain {
				refundCoins = refundCoins.Add(sdk.NewCoins(sdk.NewCoin(chainTokenInfo.MappingSymbol.String(), withdrawalOrder.GasFee.Sub(costFee))))
