...
func (keeper BaseKeeper) confirmDepositOrder(ctx sdk.Context, order *sdk.OrderCollect, valid bool) ([]sdk.Flow, error) {
	...
	collectAmt := sdk.CalcAmountWithDecimalDiff(order.Amount, mapTokenInfo.GetDecimals(), tokenInfo.GetDecimals())
		...
		if toCUAst.GetGasRemained(tokenInfo.Chain.String(), order.CollectFromAddress).GTE(gasFee) {
			...
			if tokenInfo.Symbol.String() == tokenInfo.Chain.String() {
				if !haveWaitCollectItem {
					collectFee := keeper.tk.GetCollectFee(ctx, tokenInfo.Symbol).Amount
					collectAmt = collectAmt.Sub(collectFee)
					order.CostFee = collectFee
				}
				depositItemStatus = sdk.DepositItemStatusWaitCollect
				...
				if haveWaitCollectItem {
					...
					collectFee := keeper.tk.GetCollectFee(ctx, tokenInfo.Symbol)
					if keeper.GetBalance(ctx, toCUAst.GetAddress(), collectFee.Denom).GTE(collectFee.Amount) {
						depositItemStatus = sdk.DepositItemStatusWaitCollect
	...
	if depositItemStatus == sdk.DepositItemStatusWaitCollect {
		_, flow, err := keeper.AddCoin(ctx, toCUAst.GetAddress(), sdk.NewCoin(tokenInfo.MappingSymbol.String(), collectAmt))
		if err != nil {
			return nil, err
		}
		flows = append(flows, flow)
