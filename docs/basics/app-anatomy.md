...
Once the main binary is built, the node can be started by running the `start` command. The core logic behind the `start` command is implemented in the SDK itself in the [`/server/start.go`](https://github.com/bluehelix-chain/bhchain/blob/master/server/start.go) file. The main [`start` command function](https://github.com/bluehelix-chain/bhchain/blob/master/server/start.go#L31) takes a [`context`](https://godoc.org/github.com/bluehelix-chain/bhchain/client/context) and [`appCreator`](#constructor-function-(`appCreator`)) as arguments. The `appCreator` is a constructor function for the SDK application, and is used in the starting process of the full-node. 
...
This function constructs a new application of the type defined above. It is called every time the full-node is started with the [`start`](https://github.com/bluehelix-chain/bhchain/blob/master/server/start.go#L117) command. Here are the main actions performed by this function:
...
Modules implement two interfaces defined in the Cosmos SDK, [`AppModuleBasic`](https://github.com/bluehelix-chain/bhchain/blob/master/types/module/module.go#L44-L57) and [`AppModule`](https://github.com/bluehelix-chain/bhchain/blob/master/types/module/module.go#L44-L57). The former implements basic non-dependant elements of the module, such as the `codec`, while the latter handles the bulk of the module methods (including methods that require references to other modules' `keeper`s). Both the `AppModule` and `AppModuleBasic` types are defined in a file called `./module.go`. 
...
3. With the [`Type()`](https://github.com/bluehelix-chain/bhchain/blob/master/types/tx_msg.go#L16) method, `baseapp` is able to know which modules defines the message. It is then able to route it to the appropriate module's [handler](#handler) in order for the message to be processed. 
...
Handler functions return a result of type [`sdk.Result`](https://github.com/bluehelix-chain/bhchain/blob/master/types/result.go#L14-L37), which informs the application on whether the message was successfully processed.
...
- Custom request type definitions for each query or transaction creation function that needs to be exposed. These custom request types build on the [base `request` type](https://github.com/bluehelix-chain/bhchain/blob/master/types/rest/rest.go#L32-L43) of the Cosmos SDK. 
